...
class MoistureSensorArray
{
    public:
        MoistureSensorArray(
            ...
            uint8_t pin_sens0,
            uint8_t pin_sens1,
            std::initializer_list<std::pair<IntProperty*, uint8_t>> sensors,
            IntProperty* update_interval
        );
    ...
    private:
        ...
        std::vector<std::pair<IntProperty*, uint8_t>> sensors;
        IntProperty* update_interval;
        Debug* debug;
        uint8_t iterator = 0;
...
void MoistureSensorArray::loop()
{
    ...
    if((millis() - last_check) >= update_interval->get())
    {
        ...
        std::pair<IntProperty*, uint8_t>& sensor = sensors.at(iterator);
